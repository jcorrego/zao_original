...
class MailChimpIntegration extends Module
{
    ...
    public function uninstall()
    {
        if (!parent::uninstall() || !$this->deactivateMailchimpStore() || !$this->unregisterHook('moduleRoutes') || !$this->unregisterHook('actionProductAdd') ||
                ...
                !$this->unregisterHook('actionObjectCustomerAddAfter') || !$this->unregisterHook('actionObjectCustomerUpdateAfter') || !$this->unregisterHook('actionObjectCartUpdateAfter') || !$this->unregisterHook('displayHeader')  || !$this->unregisterHook('displayFooter')) {
            return false;
        }
    ...
    public function hookActionProductUpdate($params)
    {
        ...
        $result = $this->sendApiRequest($url, 'PATCH', $data);
        if (array_key_exists('status', $result) && $result['status'] == 404) {
            $url = 'ecommerce/stores/'.$store_id.'/products';
            $result = $this->sendApiRequest($url, 'POST', $data);
        }
        // error_log(print_r($result, true), 3, '/var/log/presta-errors.log');
    ...
    private function formatCartArray($cart_hash)
    {
        ...
        $store_currency_format = Tools::strtoupper(CurrencyCore::getCurrenciesByIdShop($this->context->shop->id)[0]['iso_code']);
    ...
    public function formatStoreArray()
    {
        ...
        $store_currency_iso = Tools::strtoupper(CurrencyCore::getCurrenciesByIdShop($store_id)[0]['iso_code']);
