...
class PayulatamResponseModuleFrontController extends ModuleFrontController
{
    ...
    {   
		...
		if ($transaction_state == 6 && $pol_response_code == 5)
			...
			$estado_tx = $payulatam->l('Transacción Pendiente, Compruebe si el débito se realizó en el Banco');
		else if ($transaction_state == 4 && $pol_response_code == 1)
		{
			$estado_tx = $payulatam->l('Transacción Aprobada');
			$messageApproved = $payulatam->l('Gracias por su compra!');
		}
		else
		{
