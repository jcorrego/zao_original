...
<form class="std clearfix" action="{$post->url|escape:'html':'UTF-8'}" method="post">
	<fieldset>
		<div class="box">
			<h3 class="page-heading bottom-indent">{l s='Enviar Comentario' mod='ph_simpleblog'}</h3>
			<div class="form-group">
				<label for="customer_name">{l s='Nombre Completo' mod='ph_simpleblog'}</label>
			...
			<div class="form-group">
				<label for="comment_content">{l s='Su comentario' mod='ph_simpleblog'}</label>
		...
		<p class="cart_navigation required submit clearfix">
			...
			<button type="submit" class="button btn btn-default button-medium" name="submitNewComment" id="submitNewComment">
				<span>
					{l s='Agregar un comentario' mod='ph_simpleblog'}
					<i class="fa fa-chevron-right right"></i>
				</span>
			</button>
			{else}
			<input type="submit" class="button" name="submitNewComment" value="{l s='Agregar un comentario' mod='ph_simpleblog'}" />
			{/if}
		</p>
	</fieldset>
</form>
